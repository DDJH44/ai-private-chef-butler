  ...
  return (
    ...
    >
      {selectMode && (
        ...
        }}><Check size={24} strokeWidth={2}/></div>
      )}
      {recipe.imageUrl && (
      ...
      <div style={{ padding: 16 }}>
        ...
        <RecipeSaveBlock content={recipe.content} compact />
        {recipe.content && stripSaveBlocks(recipe.content) && (
          <p style={{
            fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.6,
            marginBottom: 12, overflow: "hidden", display: "-webkit-box",
            WebkitLineClamp: 2, WebkitBoxOrient: "vertical",
          }}>
            {stripSaveBlocks(recipe.content)}
          </p>
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 12, color: "var(--text-muted)" }}>
          {recipe.cookingTime && (
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <Clock size={12} strokeWidth={1.8}/> {recipe.cookingTime}
            </span>
          )}
          {recipe.ingredients && recipe.ingredients.length > 0 && (
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <ChefHat size={12} strokeWidth={1.8}/> {recipe.ingredients.length} 种食材
            </span>
          )}
          {recipe.videoUrl && (
            <span style={{ display: "flex", alignItems: "center", gap: 4, color: "#fb7299" }}>
              <Video size={12} strokeWidth={1.8}/> 视频
            </span>
          )}
        </div>
